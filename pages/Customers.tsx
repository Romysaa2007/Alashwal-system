...
import { getStore, updateCustomers } from '../services/store';
        ...
        {customers.map(c => (
          <div key={c.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm relative overflow-hidden group">
            ...
            <div className="flex gap-2">
                ...
                <button className="p-3 bg-slate-100 text-slate-600 rounded-xl hover:bg-slate-200">📂</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {selectedCustomer && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[400]">
          <div className="bg-white rounded-[2rem] w-full max-w-md p-8 shadow-2xl">
            ...
            <p className="text-slate-500 mb-6 font-bold">إجمالي المديونية: {selectedCustomer.totalDebt} ج.م</p>
            <div className="space-y-4">
              ...
              <button onClick={handlePayment} className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-black text-lg shadow-xl shadow-emerald-100">تأكيد الاستلام</button>
